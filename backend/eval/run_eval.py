...
Usage:
    python -m backend.eval.run_eval                 # deterministic scorers only
...
def main():
    ...
    for lbl in labels:
        ...
        if cid not in claims:
            ...
            continue
        claim_dict = claims[cid]
        claim = Claim(**claim_dict)
        print(f"[run] {cid} ({claim.claim_type}, {claim.insurance_provider})...", flush=True)
        result = review_claim(claim)

        case: dict = {"claim_id": cid, "ambiguity": lbl.get("ambiguity")}
    ...
    for c in per_case:
        ...
        print(f"  [{status}] {c['claim_id']}: expected={c['expected']:32s} actual={c['actual']:32s}{judge}")
    print(f"\nWrote: {args.out}")
